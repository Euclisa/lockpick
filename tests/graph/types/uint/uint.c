...
{                                                                                                                               \
    ...
    {                                                                                                                           \
        ...
        size_t b_width = rand() % width_high;                                                                                   \
        size_t res_width = rand() % width_high;                                                                                 \
        LP_TEST_STEP_INTO(__test_graph_uint_##op_type##_width_set(a_width,b_width,res_width));                                  \
        LP_TEST_STEP_INTO(__test_graph_uint_##op_type##_width_set(0,b_width,res_width));                                        \
        LP_TEST_STEP_INTO(__test_graph_uint_##op_type##_width_set(a_width,0,res_width));                                        \
        LP_TEST_STEP_INTO(__test_graph_uint_##op_type##_width_set(a_width,b_width,0));                                          \
...
{                                                                                                                               \
    ...
    {                                                                                                                           \
        size_t a_width = rand() % width_high;                                                                                   \
        size_t b_width = rand() % width_high;                                                                                   \
        LP_TEST_STEP_INTO(__test_graph_uint_##op_type##_inplace_width_set(a_width,b_width));                                    \
        LP_TEST_STEP_INTO(__test_graph_uint_##op_type##_inplace_width_set(0,b_width));                                          \
...
#define TEST_GRAPH_UINT_SHIFT_OP_DANGLING_INIT_CHUNK(res_width)                                                                 \
    lpg_graph_t *graph = lpg_graph_create("test",a_width,res_width,__LPG_TEST_UINT_MAX_GRAPH_NODES);                            \
    lpg_uint_t *graph_a = lpg_uint_allocate_as_buffer_view(graph,graph->inputs,a_width);                                        \
    lpg_uint_t *graph_res_obt = lpg_uint_allocate_as_buffer_view(graph,graph->outputs,res_width);

#define TEST_GRAPH_UINT_SHIFT_OP_CLEAN_UP_CHUNK                                                                                 \
    lpg_graph_release(graph);                                                                                                   \
    lpg_uint_release(graph_a);                                                                                                  \
    lpg_uint_release(graph_res_obt);                                                                                            \
    free(original_hex_str);                                                                                                     \
...
#define TEST_GRAPH_UINT_SHIFT_OP_DANGLING_CLEAN_UP_CHUNK                                                                        \
    lpg_graph_release(graph);                                                                                                   \
    lpg_uint_release(graph_a);                                                                                                  \
    lpg_uint_release(graph_res_obt);
...
#define __TEST_GRAPH_UINT_SHIFT_OP_GEN_AND_TEST(op_type,postfix)                                                                \
static inline void __test_graph_uint_gen_and_test_##op_type##_##postfix(                                                        \
        lpg_graph_t *graph,                                                                                                     \
        lpg_uint_t *graph_a,                                                                                                    \
        size_t shift,                                                                                                           \
...
{                                                                                                                               \
    char *hex_str_a = (char*)malloc(MAX_HEXES_NUM+1);                                                                           \
    cases_uint_t a_prop,res_true_prop,res_obt_prop;                                                                             \
    /* Generate samples */                                                                                                      \
    lp_uint_rand(a_prop,graph_a->width);                                                                                        \
    lp_uint_to_hex(a_prop,hex_str_a,MAX_HEXES_NUM);                                                                             \
    lp_uint_##op_type(a_prop,shift,res_true_prop);                                                                              \
    /* Build mask for truncating result to appropriate width */                                                                 \
    cases_uint_t __res_mask,__one;                                                                                              \
    lp_uint_from_hex(__res_mask,"1");                                                                                           \
    lp_uint_from_hex(__one,"1");                                                                                                \
...
#define TEST_GRAPH_UINT_SHIFT_OP(op_type, width_sets_num, width_high, cases_num)                                                \
__TEST_GRAPH_UINT_SHIFT_OP_GEN_AND_TEST(op_type,plain)                                                                          \
void __test_graph_uint_##op_type(size_t a_width, size_t shift, size_t res_width)                                                \
{                                                                                                                               \
    const uint32_t tests_num = cases_num;                                                                                       \
    TEST_GRAPH_UINT_SHIFT_OP_INIT_CHUNK(res_width)                                                                              \
    lpg_uint_##op_type(graph_a,shift,graph_res_obt);                                                                            \
    for(uint32_t test_i = 0; test_i < tests_num; ++test_i)                                                                      \
    {                                                                                                                           \
        LP_TEST_STEP_INTO(                                                                                                      \
            __test_graph_uint_gen_and_test_##op_type##_plain(                                                                   \
                graph,graph_a,shift,graph_res_obt,                                                                              \
                original_hex_str,converted_hex_str)                                                                             \
        );                                                                                                                      \
    }                                                                                                                           \
    lp_test_cleanup:                                                                                                            \
...
{                                                                                                                               \
    TEST_GRAPH_UINT_SHIFT_OP_DANGLING_INIT_CHUNK(res_width)                                                                     \
    lpg_uint_##op_type(graph_a,shift,graph_res_obt);                                                                            \
    size_t dangling_nodes = lpg_graph_count_dangling_nodes(graph);                                                              \
    LP_TEST_ASSERT(dangling_nodes == 0,                                                                                         \
...
{                                                                                                                               \
    ...
    {                                                                                                                           \
        size_t a_width = rand() % width_high;                                                                                   \
        size_t res_width = rand() % width_high;                                                                                 \
        size_t shift = rand() % MAX(a_width,res_width);                                                                         \
        LP_TEST_STEP_INTO(__test_graph_uint_##op_type##_width_set(a_width,shift,res_width));                                    \
        LP_TEST_STEP_INTO(__test_graph_uint_##op_type##_width_set(0,shift,res_width));                                          \
        LP_TEST_STEP_INTO(__test_graph_uint_##op_type##_width_set(a_width,0,res_width));                                        \
...
#define TEST_GRAPH_UINT_SHIFT_OP_INPLACE(op_type, width_sets_num, width_high, cases_num)                                        \
__TEST_GRAPH_UINT_SHIFT_OP_GEN_AND_TEST(op_type,inplace)                                                                        \
void __test_graph_uint_##op_type##_inplace(size_t a_width, size_t shift)                                                        \
{                                                                                                                               \
    const uint32_t tests_num = cases_num;                                                                                       \
    TEST_GRAPH_UINT_SHIFT_OP_INIT_CHUNK(a_width)                                                                                \
    lpg_uint_copy(graph_res_obt,graph_a);                                                                                       \
...
{                                                                                                                               \
    TEST_GRAPH_UINT_SHIFT_OP_DANGLING_INIT_CHUNK(a_width)                                                                       \
    lpg_uint_copy(graph_res_obt,graph_a);                                                                                       \
    lpg_uint_##op_type##_ip(graph_res_obt,shift);                                                                               \
    size_t dangling_nodes = lpg_graph_count_dangling_nodes(graph);                                                              \
    LP_TEST_ASSERT(dangling_nodes == 0,                                                                                         \
...
TEST_GRAPH_UINT_SHIFT_OP(lshift,20,64,10)
TEST_GRAPH_UINT_SHIFT_OP_INPLACE(lshift,20,64,10)

TEST_GRAPH_UINT_SHIFT_OP(rshift,20,64,10)
TEST_GRAPH_UINT_SHIFT_OP_INPLACE(rshift,20,64,10)
...
void lp_test_graph_uint()
{
    ...
    LP_TEST_RUN(test_graph_uint_lshift());
    LP_TEST_RUN(test_graph_uint_lshift_inplace());

    LP_TEST_RUN(test_graph_uint_rshift());
    LP_TEST_RUN(test_graph_uint_rshift_inplace());
}